void setup() {
  ...
  // Защита от заведомо "мусорных" показаний (например, при сбойном/
  // непринятом чтении по I2C - именно так выглядят те самые случайные
  // числа вместо нормального времени): если что-то в прочитанном
  // времени в принципе невозможно, считаем это неудачным чтением и
  // принудительно записываем время синхронизации ещё раз.
  boolean rtcGarbage = (now.year() < 2000 || now.year() > 2099 ||

...
#define BLANK_TICKS 5   // доп. тиков полного гашения ПЕРЕД сменой декодера/лампы (~128 мкс каждый).
                         // Даёт анодной цепи уже выключенной лампы больше времени полностью
                         // разрядиться/погаснуть до того, как общий декодер (один на все 4 лампы)
                         // переключится на цифру следующей - это и вызывало слабое "фантомное"
                         // проявление невыбранной цифры в момент смены ламп.
ISR(TIMER2_COMPA_vect) {
  ...
  if (indiCounter[curIndi] > 25 + BLANK_TICKS) {  // порог в 25 единиц + гасящий интервал
